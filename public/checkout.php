 ...
 <form action="create-order.php" method="POST" class="mx-5">
     ...
     <div class="form-group">
         ...
         <input type="password" class="form-control" id="inputPassword" name="password"
             <?= !empty($user) ? 'readonly' : '' ?>
             value="<?= isset($user['password']) ? htmlentities($user['password']) : ''?>">
     </div>
     <div class="form-group">
         <label for="inputEmail">Email</label>
         <input type="text" class="form-control" id="inputEmail" name="email" <?= !empty($user) ? 'readonly' : '' ?>
             value="<?= isset($user['email']) ? htmlentities($user['email']) : ''?>">
     </div>
     <div class="form-group">
         <label for="inputPhone">Phone</label>
         <input type="text" class="form-control" id="inputPhone" name="phone" <?= !empty($user) ? 'readonly' : '' ?>
             value="<?= isset($user['phone']) ? htmlentities($user['phone']) : ''?>">
     </div>
     <div class="form-group">
         <label for="inputAddress">Address</label>
         <input type="text" class="form-control" id="inputAddress" name="street" <?= !empty($user) ? 'readonly' : '' ?>
             value="<?= isset($user['street']) ? htmlentities($user['street']) : ''?>">
     </div>
     <div class="form-group col-md-2">
         <label for="inputZipcode">Zip code</label>
         <input type="text" class="form-control" name="postalCode" id="inputZipcode"
             <?= !empty($user) ? 'readonly' : '' ?>
             value="<?= isset($user['postal_code']) ? htmlentities($user['postal_code']) : ''?>">
     </div>
     <div class="form-group">
         <label for="inputCity">City</label>
         <input type="text" class="form-control" name="city" id="inputCity" <?= !empty($user) ? 'readonly' : '' ?>
             value="<?= isset($user['city']) ? htmlentities($user['city']) : ''?>">
     </div>
     <div class="form-row">
